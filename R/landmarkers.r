...
nb.landmarker <- function (dataset, data.char) {
  formul <- constroi.formula(dataset[[1]]$attributes$target.attr, dataset[[1]]$attributes$attr.name[-length(dataset[[1]]$attributes$attr.name)])
...
nb.landmarker.correlation <- function (dataset, data.char) {
  formul <- constroi.formula(dataset[[1]]$attributes$target.attr,
...
lda.landmarker.correlation <- function (dataset, data.char) {
  ...
  for (e in 1:10) {
    ...
    if (length(new.vars)==0) {
      formul <- constroi.formula(dataset[[1]]$attributes$target.attr, vars)
    } else {
      formul <- constroi.formula(dataset[[1]]$attributes$target.attr, new.vars)
    }
